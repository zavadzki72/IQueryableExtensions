...
namespace PerformanceTest
{
    public class SearchTeamPerformance
    {
        ...
        private readonly List<Team> _teams = Utils.GetTeamListRandom(5000000);

        [Benchmark]
        public List<Team> GetTeamsWithoutQueryableExtensions()
        {
            var filter = new GetTeamFilterDto();
            var query = from t in _teams select t;
        ...
        {
            return _teams.AsQueryable().Apply(new GetTeamFilterDto()).ToList();
        }
        ...
        {
            return _teams.AsQueryable().Apply(_filter).ToList();
        }
